fn_SyncGroupClient = {
	...
	if(_GroupType in ["[포병]"] && ({ _GroupType == (_x getVariable['GroupType',(Group_Types select 0)]) && _x != (group player) } count (["GetAllGroupsOfSide",[GRLIB_side_friendly]] call BIS_fnc_dynamicGroups)) > 0) exitWith { hint "동일한 종류의 지원분대는 2개 이상 창설 할 수 없습니다."; };
