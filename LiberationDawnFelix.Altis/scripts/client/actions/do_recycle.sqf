...
if ( dorecycle == 1 && !(isnull _vehtorecycle) && alive _vehtorecycle) then {
	...
	[gamelogic, format["%1 님에 의해 FOB %2 의 %3가 재활용되었습니다", name player, [[] call F_getNearestFob] call F_getFobName, getText (_cfg >> (_objectinfo select 0) >> "displayName" ) ]] remoteExec ["globalChat",[WEST,civilian]];
};