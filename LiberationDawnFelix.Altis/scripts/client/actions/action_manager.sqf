...
private [ "_idact_build",  "_idact_arsenal", "_idact_buildfob", "_idact_redeploy", "_idact_tutorial", "_distfob", "_distarsenal",  "_distbuildfob", "_distspawn", "_distredeploy", "_idact_commander", "_idact_exchscore" ];
...
*/
	...
	if ( (_fobdistance < _distredeploy || count _nearspawn != 0 || (player distance lhd) < 200) && alive player && vehicle player == player && score player > 0) then {
		if ( _idact_exchscore == -1 ) then {
			_idact_exchscore = player addAction ["<t color='#80FF80'>" + localize "STR_EXCHANGE_SCORE" + "</t>","scripts\client\actions\exchange_score.sqf","",-750,false,true,"","build_confirmed == 0"];
		};
	} else {
		if ( _idact_exchscore != -1 ) then {
