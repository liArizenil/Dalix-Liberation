...
*/
	...
	if ( ( count GRLIB_all_fobs > 0 ) && ( GRLIB_endgame == 0 ) && (_fobdistance < _distredeploy || (player distance lhd) < 200) && alive player && vehicle player == player && ( ( [ player, 5 ] call F_fetchPermission ) || ( player == ( [] call F_getCommander ) || [] call F_isAdmin ) ) && _GroupJoined ) then {
	...
	if(!_GroupJoined) then {
		...
		if (isNil{ ((group player)getVariable['GroupType',nil]) } && leader group player == player) then {
			["<t color='#ff0000' size = '.9'>분대 태그를 지정하십시오.</t><br />적절한 분대 태그를 설정해 주시기 바랍니다.</t>",-1,0.1,3,0,0,789] spawn BIS_fnc_dynamicText;
		};
	};
