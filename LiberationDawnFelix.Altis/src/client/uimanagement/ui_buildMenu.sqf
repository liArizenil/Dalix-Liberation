...
if ( ( [ getpos player , 500 , CONST_SIDE_OPFOR ] call F_getUnitsCount ) > 4 ) exitWith { hint localize "STR_BUILD_ENEMIES_NEARBY"; };
...
while { dialog && alive player && (dobuild == 0 || buildtype == 1)} do {
	...
	if (dobuild == 0 && _selected_item != -1 && (_selected_item < (count _build_list))) then {
		...
		if ( buildtype != 8 ) then {
			{ if ( ( _build_item select 0 ) == ( _x select 0 ) ) exitWith { _base_link = _x select 1; _linked = true; } } foreach ELITE_VEHICLES_LINK;

			if ( _linked ) then {
				if ( !(_base_link in SECTOR_BLUFOR) ) then { _linked_unlocked = false };
			};
		};
	};
