if (isServer) then {
	...
	if ( endgame == 0 ) then {
		if ( (!( _liberated_sector in sectors_tower )) && (((random (200.0 / (CONST_DIFFICULTY_MODIFIER * CONST_CSAT_AGGRESSIVITY) )) < (combat_readiness - 20)) || ( _liberated_sector in SECTOR_BIGTOWN )) && ([] call F_opforCap < GRLIB_battlegroup_cap) ) then {
